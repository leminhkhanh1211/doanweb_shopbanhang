...
class PostController extends Controller
{
    ...
    public function all_post(){
      $this->AuthLogin(); // Nếu có kiểm tra đăng nhập
 ...
 public function edit_post($post_id){
  $cate_post = CateGory_Post::orderBy('cate_post_id')->get();
  $post = Post::find($post_id);
 ...
 // $catepost = Category_Post::where('cate_post_slug',$post_slug)->take(1)->get();
  ...
  $related = Post::with('cate_post')->where('post_status',0)->where('cate_post_id',$cate_post_id)->whereNotIn('post_slug',[$post_slug])
  ->take(5)->get();

  return view('pages.baiviet.baiviet')->with('category',$cate_product)
  ->with('brand',$brand_product)
  ->with('meta_desc',$meta_desc)->with('meta_keywords',$meta_keywords)->with('meta_title',$meta_title)->with('url_canonical',$url_canonical)
  ->with('slider',$slider)->with('post',$post)->with('category_post',$category_post)->with('related',$related);
 }
